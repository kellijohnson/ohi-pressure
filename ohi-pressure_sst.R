...
###############################################################################
# Data was obtained from nodc.noaa.gov/sog/cortad V5, which is a NetCDF file
# doi:10.7289/V5CZ3545
# The nc file has three dimensions (time, latitude, and longitude), where the
...
rasterOptions(dir.temp = dir.temp)
...
for(i in 1:53) {
  ...
  sd <- calc(s, fun = function(x){sd(x, na.rm = TRUE)},
    progress = "text", filename = paste("sd_sst_week_", i,".tif"))
}
...
foreach(i = 1986:2008) %dopar% {
  ...
  diff <- overlay(s, ref,
    fun = function(x,y){x-y}) %>% #calc diff between recent 5 year mean and ref
